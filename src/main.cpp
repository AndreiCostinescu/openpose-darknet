...
void render_pose_keypoints
    ...
    for (int person = 0; person < keyshape[0]; ++person) {
        cout << "At person " << person << endl;
        // Draw lines
        for (int pair = 0u; pair < pairs_size; pair += 2) {
            cout << "\tAt pair " << pair << "/" << pairs_size << ":" << endl;
...
void connect_bodyparts
    ...
    cout << "In connect Bodyparts..." << endl;
    cout << pose_keypoints.size() << ", " << mapw << ", " << maph << ", " << inter_min_above_th << ", " << inter_th
         << ", " << min_subset_cnt << ", " << min_subset_score << endl;
    ...
    cout << num_body_parts << ", " << num_body_part_pairs << ", " << subset_counter_index << ", " << subset_size << ", "
         << peaks_offset << ", " << map_offset << endl;

    for (unsigned int pair_index = 0u; pair_index < num_body_part_pairs; ++pair_index) {
        cout << "At " << pair_index << "/" << num_body_part_pairs << endl;
        ...
        cout << body_partA << ", " << body_partB << ", " << nA << ", " << nB << endl;
        ...
        } else // if (nA != 0 && nB != 0)
        {
            ...
            cout << "limb_idx[2 * pair_index] * map_offset = " << limb_idx[2 * pair_index] * map_offset << endl;
            const float *const mapX = map + limb_idx[2 * pair_index] * map_offset;
            // limb PAF y-direction heatmap
            cout << "limb_idx[2 * pair_index + 1] * map_offset = " << limb_idx[2 * pair_index + 1] * map_offset << endl;
            ...
            for (int i = 1; i <= nA; i++) {
                for (int j = 1; j <= nB; j++) {
                    ...
                    if (norm_vec > 1e-6) {
                        ...
                        cout << "sX = " << sX << "; sY = " << sY << "; vecX = " << vecX << "; vecY = " << vecY << endl;
                        float sum = 0.;
                        int count = 0;
                        for (int lm = 0; lm < num_inter; lm++) {
                            ...
                            cout << "idx = " << idx << endl;
                            cout << "mapX[idx] = " << mapX[idx] << endl;
                            cout << "mapY[idx] = " << mapY[idx] << endl;
...
int main
    ...
    int net_outw = 0;
    int net_outh = 0;
    initNet("../coco.data", cfg_path, weight_path, 0, &net_inw, &net_inh, &net_outw, &net_outh);
    ...
    float *netoutdata = runNet(netin_data);
    ...
    float *heatmap_peaks = new float[heatmapSize];
